...
export function AdminRestaurantPage({ restaurantId, onClose }: Props) {
  ...
  const fetchRestaurant = useCallback(async () => {
    try {
      ...
      setRestaurant(data);
      setCommentDraft(data.adminComment ?? "");
      setError(null);
    } catch {
      ...
      <div className={onClose ? "flex-1 overflow-y-auto p-5" : "max-w-5xl mx-auto md:px-6 pt-5 md:pt-4"}>
        <div className="space-y-4">
          ...
          <div className="bg-card border border-border rounded-xl p-4 space-y-3">
            <div className="text-sm font-semibold text-foreground">Send email</div>
            {EMAIL_TEMPLATES.map((tpl) => {
              ...
              const sentLabel = sentAt
                ...
                : null;
              return (
                <div key={tpl.id} className="flex items-center justify-between gap-3 bg-secondary/40 border border-border rounded-lg p-3">
                  <div className="min-w-0 flex-1">
                    ...
                    {sentLabel ? <div className="text-[11px] text-emerald-600 mt-1">✓ {sentLabel}</div> : null}
                  </div>
                  <button
                    type="button"
                    onClick={() => setConfirmTemplate(tpl)}
                    disabled={sendingTemplate === tpl.id}
                    className="h-8 px-3 text-xs font-medium text-foreground bg-secondary border border-border rounded-md shrink-0 disabled:opacity-60"
                  >
          ...
          <div className="bg-card border border-border rounded-xl p-4 space-y-3">
            ...
            <div className="flex justify-end">
              <button
                type="button"
                onClick={saveComment}
                disabled={savingComment || commentDraft === (restaurant.adminComment ?? "")}
                className="inline-flex items-center gap-1.5 h-8 px-3 text-xs font-medium text-primary-foreground bg-primary-gradient rounded-lg disabled:opacity-50"
              >
                {savingComment ? <span className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin" /> : null}
                Save
              </button>
            </div>
          </div>
...
function AutoGrowTextarea({
  ...
  return (
    <textarea
      ...
      rows={3}
      className="w-full min-h-[80px] bg-secondary border border-border rounded-lg p-3 text-sm text-foreground placeholder:text-muted-foreground resize-none outline-none focus:border-foreground/40 overflow-hidden"
    />
  );
}
