    ...
    try {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          if (j?.message === "Device is revoked") msg = "This device was revoked. Generate a new code.";
        } catch {
          // ignore
        }
        setError(msg);
  ...
  return (
    <div className="min-h-dvh bg-background flex items-center justify-center px-6">
      <div className="w-full max-w-md text-center">
        <h1 className="text-3xl font-medium text-foreground mb-2">Pair this tablet</h1>
        <p className="text-sm text-muted-foreground leading-snug mb-8">
          Enter the 6-digit code from <span className="font-medium text-foreground">Settings → Devices</span> in the admin dashboard.
        </p>
        ...
        {submitting ? (
          ...
          <div className="text-xs text-muted-foreground">
            Code expires after 2 minutes. Generate a new one in the admin if needed.
          </div>
        )}
      </div>
    </div>
